...
int CDecodeRnx302::decode(char* infiles, prcopt opt) {
    ifstream in(infiles);
    if(!in) {
...
int CDecodeRnx302::scanobsfile(ifstream &in) {
    int count = 0;
    string line;
    const char* p;
    while (!in.eof()) {
...
int CDecodeRnx302::readobsh(ifstream &in, prcopt opt) {
    const char* p;
    while(!in.eof()) {
        string line, label;
        getline(in, line);
...
int CDecodeRnx302::readfreqtype(ifstream &in, string line, int sys, prcopt opt) {
...
int CDecodeRnx302::readobsb(ifstream &in, int obsnum, prcopt opt) {
...
int CDecodeRnx302::decodeEpoch(int sitenum, int &epoch, string line) {
...
bool CDecodeRnx302::decodeobsr(ifstream &in, int sitenum, int satnum, prcopt opt, int &epoch) {
    string line;
    const char* p;
...
int CDecodeRnx302::code2sys(char code) {
    if (code == 'G') return SYS_GPS;
    else if (code == 'C') return SYS_BDS;
    else return -1;
}

int CDecodeRnx302::code2freqnum(int pos) {
...
int CDecodeRnx302::readephh(ifstream &in, prcopt opt) {
    string line, label;
    while (!in.eof()) {
        getline(in, line);
        try {
...
int CDecodeRnx302::readephb(ifstream &in, prcopt opt) {
    int ephnum = 0;
    string line;
    while(!in.eof()) {
        getline(in, line);
...
int CDecodeRnx302::readgpseph(ifstream &in, string& line, prcopt opt, int ephnum) {
...
int CDecodeRnx302::readbdseph(ifstream &in, string &line, prcopt opt, int ephnum) {
    int year, month, day, hour, min;
    double sec;
...
int CDecodeRnx302::scannav(ifstream &in, prcopt opt) {
    string line;
    int count = 0;
    while(!in.eof()) {
        getline(in, line);
